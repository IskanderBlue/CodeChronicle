...
vendored rather than imported for the same reason ``config.synonyms`` is: CCM
...
* **No provision-id or table-id pass.**  CCM adds dotted ids as terms because a
  ...
  ``api.llm_parser`` extracts ids with its own regexes and hands them to the
...
* ``TestNoDriftFromCCM`` in ``api/tests/test_query_keywords.py`` runs the real
