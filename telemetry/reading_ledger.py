...
``web.api.provisions.record_fetch`` does this for the API, one text to a request.
...
def keys_for(
    ...
    The key is the same tuple ``web.api.provisions.record_fetch`` writes:
...
def record_reads(
    ...
    *,
    ...
    ``telemetry.insights.ledger_health``, which counts signed-in readers only.
    """
    try:
...
def record_versions(
    ...
    *,
    ...
    view event it writes anyway, and ``telemetry.insights.ledger_health`` reads the
    pair back.
