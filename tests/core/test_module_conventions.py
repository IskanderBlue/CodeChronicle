...
1. **No import inside a function body.**  A lazy import hides a dependency the
   ...
   ``apps.py`` is exempt, because ``AppConfig.ready()`` runs after the
   app registry is built.
2. **No relative import.**  ``from .regulation import x`` and
   ``from web.views.regulation import x`` name the same module, and only the
   ...
   ``web/views`` and the relative form was hiding every one of them.
...
Rules 4 and 5 exempt ``ALLOWED_IMPORTS`` in a package ``__init__.py``, because
every package must bind that one name for
``tests/structure/test_import_manifests.py`` to read it.  The exemption is
derived from the shape, not from a list of files.
...
def test_no_function_body_imports() -> None:
    """Every import sits at module level, except inside ``AppConfig.ready``.

    The exemption is derived from the file's name, not from a list of files.
    Django calls ``ready()`` once the app registry is populated, and that is
    the only correct moment to import a module which touches a model or
    registers a signal receiver; a module-top import there runs before the
    registry exists and raises ``AppRegistryNotReady``.  Django's own
    documentation prescribes the deferred import, so this is the framework's
    rule.  Naming the shape rather than the files means a new app carries the
    same exemption without anybody editing this test.
    ...
    dependency (see ``search/engine/engine.py`` and rapidfuzz).
    """
    offenders: list[str] = []
    ...
    assert not offenders, (
        ...
        "shared/html.py and core/code_names.py do).  If the dependency is "
        "genuinely optional, guard it at module top with try/except "
        "ImportError.  The only exempt shape is an apps.py, because "
        "AppConfig.ready() runs after the app registry is built."
    )
...
def test_no_relative_imports() -> None:
    ...
    ``web/views/__init__.py`` re-exports sixteen modules and is the obvious
    ...
    assert not offenders, (
        ...
        "'from web.views.regulation import x', not 'from .regulation import "
        "x'.  A package's own __init__.py is not an exception; it imports "
        "'web.views.pages' the same way anything else does."
    )
...
def _is_package_manifest(path: Path, name: str) -> bool:
    """``ALLOWED_IMPORTS`` in a package's own ``__init__.py``.

    Every package declares one (see
    ``tests/structure/test_import_manifests.py``), so eight modules bind the
    same name and two of them bind the same empty tuple.  Both are the
    manifest doing its job: the name has to be the same in every package for
    the guard to find it, and a package that imports nothing local can only
    say so with an empty tuple.  The exemption is read off the shape — this
    name, in a package ``__init__`` — so a ninth package needs no edit here,
    and the same name anywhere else is still refused.
    """
    return path.name == "__init__.py" and name == "ALLOWED_IMPORTS"
            ...
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                for target in node.targets:
                    if isinstance(target, ast.Name) and _is_constant_name(target.id):
                        if not _is_package_manifest(path, target.id):
                            by_name[target.id].append(_rel(path))
            elif isinstance(node, ast.AnnAssign) and isinstance(node.target, ast.Name):
                if _is_constant_name(node.target.id):
                    if not _is_package_manifest(path, node.target.id):
...
def test_no_duplicate_top_level_names() -> None:
    ...
    * A **management command's ``Command``**, recognised by its base class
    ...
    raise AssertionError(
        ...
        "it in one module both sides import (shared/email.py and "
        "shared/ip.py are that shape) and delete the copies.  Different "
    ...
    for path in app_files():
        for node in _parse(path).body:
            ...
            for target in targets:
                if target.id.startswith("__") or _is_package_manifest(path, target.id):
                    continue
...
def test_no_duplicate_constant_values() -> None:
    ...
    copy-pasted under two different names.  ``corpus.cross_refs`` and
    ``search.formatters.formatters`` each held their own tag-span pattern, already diverged
