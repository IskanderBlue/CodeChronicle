...
from telemetry.insights import (
    CHART_HEIGHT,
    backup_state,
    collect_metrics,
    conversion_rates,
...
class TestInsightsView:
    def test_anonymous_is_redirected_to_login(self, client):
        response = client.get(reverse("web:insights"))
    ...
    def test_window_falls_back_to_the_default_for_an_unoffered_value(self, client):
        ...
        client.force_login(user)
        for raw in ("7", "abc", ""):
            response = client.get(reverse("web:insights"), {"days": raw})
            assert response.context["days"] == 90
        response = client.get(reverse("web:insights"), {"days": "30"})
