...
class TestSearchViewPlumbing:
    ...
    def test_posted_floor_is_persisted_and_passed_down(self, client, monkeypatch):
        seen: dict = {}
        monkeypatch.setattr("web.views.search.run_search", _stub_search(seen))
    ...
    def test_signed_in_floor_round_trips_through_the_view(self, client, monkeypatch):
        ...
        client.post(
            reverse("web:search_results"), {"query": "fire", "match_threshold": "1.42"}
        )
        user.refresh_from_db()
        ...
        seen.clear()
        client.post(reverse("web:search_results"), {"query": "guards"})
