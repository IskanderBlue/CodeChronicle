"""Tokens on the mirrored assets (``corpus.asset_signing``).

The exposure being closed: the edge Worker serves R2 directly, so Django is
not in the path and ``accounts.access`` never sees a request for a scan.  The
...
from corpus.asset_signing import (
    TOKEN_PARAM,
    asset_token,
    asset_url,
    needs_token,
...
from data.assets import MIRRORED_PREFIXES, SIGNED_PREFIXES
