...
from corpus.sitemaps import ProvisionSitemap
...
class TestProvisionPageMeta:
    ...
    def test_the_canonical_url_is_the_url_the_sitemap_submits(self, provision):
        """The rule lives in corpus.seo; corpus.sitemaps implements it set-based."""
...
class TestTheIntervalAgreesWithTheVisibleWindow:
    ...
    the page title.  Both of those compare two ``corpus.seo`` outputs, so a
    conversion that moved *inside* ``corpus.seo`` keeps them agreeing while the
    page disagrees with itself.

    The window a reader sees on the exhibit does not come from ``corpus.seo``.
    It comes from :func:`corpus.citations.in_force_phrase`, and CLAUDE.md records
    ...
    def _exhibit_window(self, client):
        ...
        page also carries a ``corpus.seo`` window in a meta tag, and a loose
