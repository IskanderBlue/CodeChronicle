...
from accounts.pricing import checkout_is_configured, get_pro_price
...
from web.views.pages import _pricing_plans


def _make_price(price_id: str, **overrides):
    """A dj-stripe Price row shaped like the live one.

    ``stripe_data`` is what ``accounts.pricing`` reads; the concrete columns
...
class TestPricingPage:
    def test_the_page_prints_the_stripe_figure_with_the_currency(
        ...
        body = client.get(reverse("web:pricing")).content.decode()
    ...
    def test_the_page_quotes_no_figure_with_no_stripe_row(self, client, settings):
        ...
        response = client.get(reverse("web:pricing"))
...
class TestCheckoutWithNoPriceConfigured:
    ...
    def test_it_sends_the_reader_back_to_a_whole_pricing_page(self, client, settings):
        ...
        response = client.post(reverse("web:create_checkout_session"), follow=True)
        body = response.content.decode()

        assert response.redirect_chain[-1][0] == reverse("web:pricing")
        ...
        # The page renders through web.views.pages.pricing, so it carries its
