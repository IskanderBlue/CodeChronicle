...
from accounts.reacceptance import ACCEPTED_VERSIONS_SESSION_KEY, outstanding
...
class TestTheWall:
    def setup_method(self):
        self.client = Client()
        self.url = reverse("web:accept_terms")
    ...
    def test_the_documents_stay_readable_behind_the_wall(self, versions):
        ...
        for name in ("web:terms_of_service", "web:privacy_policy"):
    ...
    def test_an_offsite_next_is_refused(self, versions):
        ...
        assert response["Location"] == reverse("web:search")
    ...
    def test_a_current_reader_never_meets_it(self, versions):
        ...
        assert self.client.get(reverse("web:search")).status_code == 200
    ...
    def test_the_verdict_is_cached_in_the_session(self, versions):
        ...
        self.client.get(reverse("web:search"))
    ...
    def test_an_anonymous_visitor_is_unaffected(self, versions):
        assert Client().get(reverse("web:search")).status_code == 200
