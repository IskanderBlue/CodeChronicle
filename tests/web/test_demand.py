...
from web.views.demand import MAX_REQUESTS_PER_IP_PER_DAY
...
class TestEditionRequest:
    def test_records_the_need_without_an_email(self, client):
        response = client.post(
            reverse("web:edition_request"), {"code_text": "NBC 2015"}
        )
    ...
    def test_empty_need_is_refused_and_stores_nothing(self, client):
        response = client.post(reverse("web:edition_request"), {"code_text": "   "})
        assert response.status_code == 200
    ...
    def test_over_limit_posts_are_accepted_silently_and_not_stored(self, client):
        for i in range(MAX_REQUESTS_PER_IP_PER_DAY):
            client.post(
                reverse("web:edition_request"), {"code_text": f"code {i}"}
            )
        response = client.post(
            reverse("web:edition_request"), {"code_text": "one too many"}
        )
        assert response.status_code == 200
    ...
    def test_get_is_not_allowed(self, client):
        assert client.get(reverse("web:edition_request")).status_code == 405

    def test_the_band_appears_on_the_landing_page(self, client):
        body = client.get(reverse("web:about")).content.decode()
