"""The public landing page (``web.views.landing``), at ``/`` and ``/about/``.
...
# The module, not the view: ``web.views.landing`` resolves to the re-exported
# view function via ``web/views/__init__.py``.
from web.views.landing import SPECIMEN, _free_tier_examples
...
class TestLandingRouting:
    ...
    def test_root_sends_a_signed_in_reader_to_the_search_page(self, client) -> None:
        ...
        assert response.url == reverse("web:search")
    ...
    def test_about_never_redirects(self, client) -> None:
        ...
        client.force_login(user)
        response = client.get(reverse("web:about"))
        assert response.status_code == 200
        assert "landing.html" in [t.name for t in response.templates]

    def test_search_page_still_serves_the_search_template(self, client) -> None:
        response = client.get(reverse("web:search"))
        assert response.status_code == 200
    ...
    def test_try_links_carry_their_date(self, client, loaded_edition) -> None:
        ...
        body = client.get(reverse("web:landing")).content.decode()
    ...
    def test_search_page_seeds_the_date_from_the_url(self, client) -> None:
        body = client.get(f"{reverse('web:search')}?q=guards&d=2010-06-01").content.decode()
    ...
    def test_search_page_ignores_a_malformed_date(self, client) -> None:
        ...
        body = client.get(f"{reverse('web:search')}?q=guards&d=nonsense").content.decode()
    ...
    def test_hero_query_hands_off_to_the_search_page(self, client) -> None:
        ...
        question."""
        body = client.get(reverse("web:landing")).content.decode()
        assert f'action="{reverse("web:search")}"' in body
...
class TestLandingRendering:
    ...
    def test_an_unresolvable_example_renders_no_link(self, client) -> None:
        ...
        classes = client.get(reverse("web:landing")).context["difference_classes"]
...
class TestTheArticleIsReachable:
    ...
    def test_the_landing_page_links_the_article(self, client) -> None:
        ...
        assert reverse("web:guard_height") in response.content.decode()
...
class TestSectionThreeSpecimens:
    ...
    and ``corpus.cross_refs`` output against one real provision. These tests are
    ...
    def test_the_query_specimen_is_the_shipping_search_form(self, client) -> None:
        ...
        rendered = [t.name for t in client.get(reverse("web:landing")).templates]
    ...
    def test_the_rail_specimen_is_the_whole_in_force_band(
        ...
        object."""
        rendered = [
            t.name for t in client.get(reverse("web:landing")).templates
        ]
    ...
    def test_the_specimen_names_its_own_provision(
        ...
        specimen = client.get(reverse("web:landing")).context["specimen"]
    ...
    def test_source_chips_are_real_regulations(self, client, specimen_provision) -> None:
        ...
        used."""
        chips = client.get(reverse("web:landing")).context["source_chips"]
...
class TestLandingFigures:
    def test_counts_come_from_the_database(self, client, loaded_edition) -> None:
        context = client.get(reverse("web:landing")).context
    ...
    def test_until_shows_the_inclusive_last_day(self, client, loaded_edition) -> None:
        ...
        editions = client.get(reverse("web:landing")).context["editions"]
    ...
    def test_the_page_states_no_parity_counts(self, client) -> None:
        ...
        for entry in client.get(reverse("web:landing")).context["difference_classes"]:
    ...
    def test_band_spans_the_whole_covered_range(self, client, loaded_edition) -> None:
        ...
        band = client.get(reverse("web:landing")).context["band"]
        assert len(band) == 1
