"""The guard-height article (``web:guard_height``).
...
from web.management.commands.verify_guard_height_article import allows_800_mm
from web.views.guard_height import MODIFIED, OPENING, PUBLISHED, TRANSITIONS
...
def _page(client):
    response = client.get(reverse("web:guard_height"))
...
class TestTheArticleRenders:
    def test_it_is_public(self, client) -> None:
        assert client.get(reverse("web:guard_height")).status_code == 200
    ...
    def test_it_is_in_the_sitemap(self) -> None:
        from corpus.sitemaps import STATIC_PAGE_NAMES

        assert "web:guard_height" in STATIC_PAGE_NAMES
...
class TestTheArgumentSurvivesTheCorpus:
    ...
    def test_the_opening_reports_its_last_governed_day(self, client, corpus) -> None:
        ...
        the text did *not* govern.  ``corpus.seo.last_governed_day`` owns that
...
class TestTheGate:
    def test_the_gated_text_renders_for_a_logged_out_reader(
        ...
        asking ``accounts.access`` for a provision-level exception."""
        response = _page(client)
...
class TestStructuredData:
    ...
    def test_it_names_the_page_it_is_about(self, client, corpus) -> None:
        ...
        assert block["url"].endswith(reverse("web:guard_height"))
