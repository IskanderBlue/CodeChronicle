...
from web.views.feedback import MAX_REPORTS_PER_IP_PER_DAY
...
class TestFilingAReport:
    def test_anonymous_reader_can_file(self, client):
        response = client.post(reverse("web:report_problem"), _payload())
    ...
    def test_free_and_pro_readers_can_file(self, client):
        ...
        for user in (free, pro):
            ...
            client.post(reverse("web:report_problem"), _payload())
    ...
    def test_version_zero_is_stored_not_dropped(self, client):
        ...
        as a missing value."""
        client.post(reverse("web:report_problem"), _payload(version="0"))
        assert ProvisionFeedback.objects.get().version == 0

    def test_keeps_a_valid_email_and_drops_a_malformed_one(self, client):
        client.post(
            reverse("web:report_problem"), _payload(email="reader@example.com")
        )
        client.post(reverse("web:report_problem"), _payload(email="not-an-address"))
        emails = list(
    ...
    def test_an_empty_note_is_refused_and_stores_nothing(self, client):
        response = client.post(reverse("web:report_problem"), _payload(note="   "))
        assert response.status_code == 200
    ...
    def test_unknown_surface_falls_back(self, client):
        client.post(reverse("web:report_problem"), _payload(surface="not-a-surface"))
    ...
    def test_over_limit_posts_are_accepted_silently_and_not_stored(self, client):
        for i in range(MAX_REPORTS_PER_IP_PER_DAY):
            client.post(reverse("web:report_problem"), _payload(note=f"fault {i}"))
        response = client.post(
            reverse("web:report_problem"), _payload(note="one too many")
        )
        assert response.status_code == 200
    ...
    def test_get_is_not_allowed(self, client):
        assert client.get(reverse("web:report_problem")).status_code == 405
...
class TestTheTriggerOnEachSurface:
    ...
    def test_the_provision_permalink_offers_it(self, client, provision):
        ...
        :mod:`web.http_cache`.  So the page carries the fetch, and the fetch
        carries the target.
        """
    ...
    def test_the_regulation_page_offers_it(self, client, provision):
        body = client.get(
            reverse("web:regulation_detail", args=[provision["regulation"].pk])
        ).content.decode()
        assert "This looks wrong" in body
    ...
    def test_the_verification_guide_does_not_offer_it(self, client):
        ...
        assert "This looks wrong" not in client.get(
            reverse("web:verification_guide")
        ).content.decode()
...
class TestTheQueue:
    ...
    def test_the_queue_lists_a_report_and_links_to_it(self, client):
        self._staff(client)
        self._report()
        body = client.get(reverse("web:insights")).content.decode()
    ...
    def test_the_status_control_moves_a_report(self, client):
        ...
        row = self._report()
        response = client.post(
            reverse("web:feedback_status", args=[row.pk]), {"status": "fixed"}
        )
        assert response.status_code == 200
        row.refresh_from_db()
    ...
    def test_an_unknown_status_is_refused(self, client):
        ...
        row = self._report()
        response = client.post(
            reverse("web:feedback_status", args=[row.pk]), {"status": "deleted"}
        )
        assert response.status_code == 400
        row.refresh_from_db()
        assert row.status == ProvisionFeedback.Status.NEW
    ...
    def test_a_non_staff_reader_cannot_see_or_move_reports(self, client):
        ...
        user = User.objects.create_user(email="reader@example.com", password="pw")
        client.force_login(user)
        assert client.get(reverse("web:insights")).status_code == 302
        assert client.post(
            reverse("web:feedback_status", args=[row.pk]), {"status": "fixed"}
        ).status_code == 302
        row.refresh_from_db()
