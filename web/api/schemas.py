...
``search.formatters.formatters`` builds for the **templates**, which carry live Django model
...
class VersionOut(Schema):
    ...
    #: ``corpus.seo.last_governed_day`` computes it for the whole product, so an
