...
from corpus.subtrees import contents_view, walk_subtree
...
from search.engine.orchestration import identity_preview
from search.formatters.formatters import code_order_key, highlight_terms
...
from telemetry.reading_ledger import record_reads, record_versions
from web.api.schemas import flatten
...
def _push_search_url(
    ...
    **A new search earns a history entry; re-measuring one does not.**  The
    ...
    response[header] = f"{reverse('web:search')}?{urlencode(params)}"
    return response
...
#
# Caveat: the free tier is scoped to OBC 2006 (accounts.access), so it sees only the
...
def search_page(request):
    ...
    ``/`` is the public landing page (``web.views.landing``); it redirects a
...
def viewer_edition_nav(request: HttpRequest):
    ...
    — ``corpus.lineage.provision_lineage``).  Linked rows keep the in-viewer load
...
def viewer_section_content(request: HttpRequest):
    ...
    # (parity with the results-card highlight in search.formatters.formatters). Mutates the
...
def search_results(request):
    ...
    # Band 2 of the anonymous allowance (web.middleware): the search ran, but
