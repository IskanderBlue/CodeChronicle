...
from accounts.teams import access_membership, administered_organizations
from core.models import CodeEdition, Membership
from corpus.seo import TITLE_SUFFIX
from web.api.auth import API_SEARCHES_BEFORE_THROTTLE
from web.views.api_keys import MAX_ACTIVE_KEYS, NEW_TOKEN_SESSION_KEY
from web.views.billing import MAX_SELF_SERVE_SEATS, sync_subscription_status
...
def verification_guide(request):
    ...
    search results use (data from ``corpus.verification.rail_examples`` via the ``rail_legend``
...
def _pricing_plans(user: Any) -> list[dict[str, Any]]:
    ...
    Every figure comes from Stripe (``accounts.pricing``), never a literal here.
    ...
    **Team's figure is a total, not a rate.**  It is billed as two Stripe
    ...
    screen.  See ``web.views.billing.create_checkout_session`` for why two
...
#
# Free mirrors the gate's scope (accounts.access / FREE_TIER_CODE_NAMES): OBC 2006
...
# slows rather than stops (API_SEARCHES_BEFORE_THROTTLE, web/api/auth.py), so the
...
def pricing(request):
    ...
    return render(
        ...
        "pricing.html",
        {
            ...
            # mirrored Stripe row (accounts.pricing), and a number written here
...
def user_settings(request):
    ...
    # web.views.api_keys, so the section and the control agree.
