...
draws, prepared the same way by ``web.views.compare.comparison_side``.  A list of six
...
one.  Same reasoning as ``web.views.landing._specimen``, and it reuses that
page's helper (``provenance_result``) to say so in code.

**The article is not a gated surface, and it does not need an exception.**
``accounts.access`` answers one question — may this reader open this *edition* —
...
version-level axis to ``accounts.access`` for this: that module works because
...
from corpus.lineage.compare import REDLINE_FLOOR, PreparedPair, version_ref
from corpus.permalinks import provision_permalink_url
from corpus.seo import article_jsonld, last_governed_day, site_origin
from data.guard_height_article import OPENING, TRANSITIONS, Ref
from search.formatters.formatters import diff_html_content, diff_similarity
from web.http_cache import corpus_conditional
from web.views.compare import comparison_side
from web.views.regulation import provenance_result
...
def _opening(version: CodeEditionProvisionVersion, user: Any) -> dict[str, Any]:
    ...
    return {
        ...
        # ``corpus.seo.last_governed_day``, which the whole product calls rather
...
def guard_height(request: HttpRequest) -> HttpResponse:
    ...
    return render(
        ...
        "guard_height.html",
        {
            ...
            "canonical_path": reverse("web:guard_height"),
