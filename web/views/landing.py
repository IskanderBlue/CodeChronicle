...
from accounts.access import edition_allowed, free_tier_code_names
...
from web.views.search import EXAMPLE_QUERIES
...
def _resolve_examples(user: Any) -> list[dict[str, Any]]:
    ...
    for entry in DIFFERENCE_CLASSES:
        ...
        if loadable and provision is not None:
            ...
            if division:
                url = reverse(
                    "web:provision_permalink", kwargs={**kwargs, "division": division}
                )
            else:
                url = reverse("web:provision_permalink_no_division", kwargs=kwargs)
