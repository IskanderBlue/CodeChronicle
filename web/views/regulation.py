...
from corpus.citations import in_force_phrase
from corpus.cross_refs import annotate_versions, cited_by
from corpus.lineage.compare import (
    annotate_chain_comparisons,
    annotate_lineage_comparisons,
    prepared_pair,
)
from corpus.lineage.provision_lineage import (
    annotate_lineage_locks,
    annotate_lineage_titles,
    resolve_lineage,
)
from corpus.permalinks import (
...
from corpus.subtrees import contents_view, related_links, walk_subtree
from corpus.verification.verification import base_input, build_rail
from data.part_applicability import part_of
from search.formatters.formatters import (
    provenance_lines,
...
from telemetry.events import arrival_context, record_event
from telemetry.reading_ledger import record_versions
from web.http_cache import corpus_conditional
from web.views.search import in_force_versions
...
def _clause_targets(clause: RegulationClause) -> list[dict[str, Any]]:
    ...
    if clause.target_level == "regulation":
        ...
        return [{
            ...
            "url": reverse("web:regulation_detail", args=[reg.pk]) if reg else None,
            "level": "regulation",
            "version": None,
...
def _sibling_link(
    ...
    Shaped exactly like :func:`corpus.subtrees.related_links` — a ``versions`` list holding
...
def provenance_result(
    ...
    Mirrors the subset of ``search.formatters.formatters._format_single_result`` the
    ...
    # search.formatters.formatters._format_single_result: a base version's From falls back
...
def _print_response(
    ...
    *,
    ...
    from :func:`corpus.citations.in_force_phrase` (which closes an open version
    at the edition's end and states the last day actually governed), and the
    instruments come from :mod:`corpus.seo`, which already refuses to fall back
...
def provision_text(
    ...
    3 of ``tasks/complete/reading-ledger-for-the-website.md``.
...
def provision_permalink(
    ...
    # ``corpus.subtrees.CONTENTS_THRESHOLD``.  The search overlay walks the same
    ...
    # ``tasks/complete/reading-ledger-for-the-website.md``: the ledger can only be as
    ...
    if for_print:
        ...
        record_event(
            ...
            context={
                ...
                # script composes URLs.  See ``telemetry.events.arrival_context``.
    ...
    return render(request, "regulation/provision_permalink.html", {
        ...
        **contents_ctx,
        ...
        # URL. Built in corpus.seo rather than in the template because the
...
def edition_contents(request: HttpRequest, code_edition: str) -> HttpResponse:
    ...
    # the same rule corpus.seo uses to pick a canonical URL.
    entries = []
    for root in roots:
    ...
    return render(request, "regulation/edition_contents.html", {
        ...
        # first day it did not.  corpus.seo owns the conversion for the product.
