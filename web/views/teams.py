...
Every write here answers to ``accounts.teams``, which owns the seat rules.  These
...
from web.views.settings_nav import TEAM_SECTION, settings_redirect
...
def invite_to_team(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    send_invite_email(
        invite,
        request.build_absolute_uri(reverse("web:accept_invite", args=[token])),
    )
