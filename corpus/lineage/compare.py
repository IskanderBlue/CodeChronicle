...
entirely, exactly as ``corpus.permalinks`` does for the permalink itself — no
...
from corpus.lineage.provision_lineage import LineageDirection, LineageLink, resolve_lineage
...
class PreparedPair:
    ...
    def url(self) -> str:
        ...
        like an internal id.
        """
        return f"{reverse('web:compare')}?a={self.a.path}&b={self.b.path}"
...
def prepared_pair(
    *,
    ...
    costs no extra query.  ``search.formatters.formatters`` stamps all three onto every
...
def version_timeline(
    ...
    ``corpus.lineage.provision_lineage``), so a longer walk would be inventing links the
