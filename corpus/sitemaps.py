...
a provision's title.  The scope therefore comes from :mod:`accounts.access` — the
...
from accounts.access import free_tier_code_names
...
def free_tier_edition_ids() -> list[int]:
    ...
    ``accounts.access`` the only definition of a tier while still letting the
...
#: Pages that describe the product rather than the corpus.  ``web:about`` is
#: listed instead of ``web:landing`` because the two render the same view and
#: ``/`` redirects a signed-in reader away; ``/about/`` is the stable address of
#: the explanation.  ``web:search`` is the tool itself — thin for a crawler,
...
STATIC_PAGE_NAMES: tuple[str, ...] = (
    ...
    "web:pricing",
    "web:data_sources",
    "web:verification_guide",
    "web:list_punctuation",
    "web:guard_height",
