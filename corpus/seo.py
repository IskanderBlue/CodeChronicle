...
``corpus.sitemaps`` implements the same rule set-based (``DISTINCT ON`` ordered
...
def base_regulation(edition: CodeEdition) -> Regulation | None:
    ...
    Public because :mod:`corpus.citations` names the same instrument: a citation
...
def _regulation_node(reg: Regulation, origin: str) -> dict[str, Any]:
    ...
    node: dict[str, Any] = {
        ...
        "url": f"{origin}{reverse('web:regulation_detail', args=[reg.pk])}",
    }
    if reg.source_url:
...
def _edition_node(edition: CodeEdition, origin: str) -> dict[str, Any]:
    ...
    return {
        ...
        "url": f"{origin}{reverse('web:edition_chain', args=[edition.pk])}",
    }
