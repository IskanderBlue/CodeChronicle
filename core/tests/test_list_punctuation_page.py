"""The list-insertion punctuation guide (``core:list_punctuation``).
...
class TestListPunctuationPage:
    def test_renders_ungated_for_anonymous(self, client) -> None:
        response = client.get(reverse("core:list_punctuation"))
    ...
    def test_no_template_syntax_leaks_into_the_page(self, client) -> None:
        body = client.get(reverse("core:list_punctuation")).content.decode()
        for leak in ("{%", "{{", "{#"):
    ...
    def test_reachable_from_data_sources(self, client) -> None:
        body = client.get(reverse("core:data_sources")).content.decode()
        assert reverse("core:list_punctuation") in body
