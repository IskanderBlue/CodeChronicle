...
from core.access import user_is_unrestricted
from core.models import Invite, Membership, Organization, User
from core.stripe_handlers import handle_subscription_created
from core.teams import SeatError, accept_invite, invite_member, personal_organization
...
class TestTheTeamPanel:
    ...
    def test_a_member_cannot_invite_by_posting(self):
        ...
        response = self.client.post(
            reverse("core:invite_to_team", args=[firm.pk]),
    ...
    def test_the_last_administrator_cannot_be_removed(self):
        ...
        self.client.force_login(admin)
        self.client.post(reverse("core:remove_member", args=[membership.pk]))
    ...
    def test_removing_a_member_leaves_the_account(self):
        ...
        self.client.force_login(admin)
        self.client.post(
            reverse("core:remove_member", args=[Membership.objects.get(user=member).pk])
        )
...
class TestTheInvitationPage:
    ...
    def test_opening_the_link_does_not_spend_the_seat(self):
        ...
        response = self.client.get(reverse("core:accept_invite", args=[token]))
        assert response.status_code == 200
        assert not Membership.objects.filter(user=newcomer).exists()

        self.client.post(reverse("core:accept_invite", args=[token]))
    ...
    def test_somebody_who_already_pays_is_told_so(self):
        ...
        self.client.force_login(buyer)
        body = self.client.get(reverse("core:accept_invite", args=[token])).content.decode()
...
class TestWhatCheckoutSendsToStripe:
    ...
    def _post(self, monkeypatch, settings, seats: str | None):
        ...
        if seats is not None:
            data["seats"] = seats
        client.post(reverse("core:create_checkout_session"), data)
        return captured
