...
def _stub_search(**parsed):
    ...
    passed in — which is what ``services.search_service`` really does, since
...
class TestTheAddressCarriesTheBuilding:
    ...
    def test_the_address_names_the_building_the_search_ranked_by(
        ...
        monkeypatch.setattr(
            "core.views.search.run_search", _stub_search()
        )
    ...
    def test_an_occupancy_the_parser_read_reaches_the_address(
        ...
        handed it out."""
        monkeypatch.setattr(
            "core.views.search.run_search",
            _stub_search(occupancy="assembly"),
        )

        response = client.post(
            reverse("core:search_results"), {"query": "guards in a theatre"}
        )
...
class TestTheFormLearnsWhatTheParserRead:
    ...
    def test_the_parsers_reading_is_announced_to_the_form(
        ...
        response = client.post(
            reverse("core:search_results"), {"query": "guards in a house"}
        )
    ...
    def test_a_query_naming_no_building_announces_nothing(
        ...
        response = client.post(
            reverse("core:search_results"), {"query": "what is a mezzanine"}
        )
...
class TestTheModelOnlyFillsABlank:
    ...
    def _params(self, monkeypatch, posted, *, model_says):
        ...
        monkeypatch.setattr("services.search_service.parse_user_query", _parse)
        monkeypatch.setattr("services.search_service.execute_search", _execute)
