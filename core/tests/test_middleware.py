...
class TestRateLimitMiddleware:
    ...
    def test_authenticated_free_user_rate_limited(self, settings):
        """Authenticated free users should be rate limited."""
        from core.models import User
        settings.RATE_LIMIT_AUTHENTICATED = 3
        user = User.objects.create_user(email="free@example.com", password="testpass")
        # Create more searches than the limit
        for _ in range(3):
            SearchHistory.objects.create(
                user=user,
                query="q",
        ...
        assert response is not None
        assert response.status_code == 429
        assert b"Daily limit reached for free accounts" in response.content

    def test_authenticated_pro_user_not_rate_limited(self, settings):
        """Authenticated Pro users should never be rate limited."""
        from core.models import User
        settings.RATE_LIMIT_AUTHENTICATED = 3
        user = User.objects.create_user(email="pro@example.com", password="testpass")
        user.pro_courtesy = True
        user.save()
