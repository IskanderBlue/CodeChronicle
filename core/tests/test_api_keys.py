...
from core.views.api_keys import MAX_ACTIVE_KEYS
...
class TestTheKeySection:
    ...
    def test_a_subscriber_sees_the_section(self):
        ...
        body = self.client.get(reverse("core:user_settings")).content.decode()
    ...
    def test_making_a_key_shows_the_token_once(self):
        self.client.force_login(self.pro)
        self.client.post(reverse("core:create_api_key"), {"name": "takeoff"})

        first = self.client.get(reverse("core:user_settings")).content.decode()
        key = ApiKey.objects.get(user=self.pro)
        assert key.lookup in first
        assert "not shown again" in first

        second = self.client.get(reverse("core:user_settings")).content.decode()
    ...
    def test_a_free_reader_cannot_make_a_key(self):
        ...
        self.client.force_login(self.free)
        self.client.post(reverse("core:create_api_key"), {"name": "sneaky"})
    ...
    def test_the_key_count_is_capped(self):
        ...
        for i in range(MAX_ACTIVE_KEYS + 2):
            self.client.post(reverse("core:create_api_key"), {"name": f"key {i}"})
    ...
    def test_a_revoked_key_frees_a_slot(self):
        ...
        self.client.post(reverse("core:revoke_api_key", args=[doomed.pk]))
        self.client.post(reverse("core:create_api_key"), {"name": "replacement"})
        assert ApiKey.objects.filter(user=self.pro, revoked_at__isnull=True).count() == (
            MAX_ACTIVE_KEYS
        )
    ...
    def test_revoking_keeps_the_row(self):
        ...
        self.client.force_login(self.pro)
        self.client.post(reverse("core:revoke_api_key", args=[key.pk]))
        key.refresh_from_db()
    ...
    def test_nobody_revokes_somebody_elses_key(self):
        ...
        response = self.client.post(reverse("core:revoke_api_key", args=[key.pk]))
    ...
    def test_the_key_routes_refuse_a_get(self):
        ...
        self.client.force_login(self.pro)
        assert self.client.get(reverse("core:create_api_key")).status_code == 405
