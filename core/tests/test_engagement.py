...
def provision_fixtures(db, settings):
    ...
    test.  Engagement capture is orthogonal to the content gate (``core.access``,
...
class TestLockedContentTracking:
    ...
    def test_search_returning_locked_results_records_an_impression(
        ...
        monkeypatch.setattr("core.views.search.run_search", _stub)
        response = client.post(reverse("core:search_results"), {"query": "fire separation"})
        assert response.status_code == 200
    ...
    def test_search_with_nothing_locked_records_no_impression(
        ...
        monkeypatch.setattr("core.views.search.run_search", _stub)
        client.post(reverse("core:search_results"), {"query": "fire separation"})
        assert not EngagementEvent.objects.exists()
